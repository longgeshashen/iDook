...
- (void)makeToastActivityWithText:(NSString *)text {
    [self makeToastActivity:CSToastActivityDefaultPosition andText:text];
}
...
- (void)makeToastActivity:(id)position andText:(NSString *)text{
    // sanity
    UIView *existingActivityView = (UIView *)objc_getAssociatedObject(self, &CSToastActivityViewKey);
    if (existingActivityView != nil) return;
    
    UIView *activityView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CSToastActivityWidth + 50, CSToastActivityHeight + 50)];
    activityView.center = [self centerPointForPosition:position withToast:activityView];
    activityView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:CSToastOpacity];
    activityView.alpha = 0.0;
    activityView.autoresizingMask = (UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleBottomMargin);
    ...
    UILabel *textLabel = [[UILabel alloc] initWithFrame:CGRectMake(25, CSToastActivityHeight, CSToastActivityWidth, 50)];
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        if([point caseInsensitiveCompare:CSToastPositionTop] == NSOrderedSame) {
            ...
            return CGPointMake(self.bounds.size.width / 2, self.bounds.size.height / 2 - 50);
        }
